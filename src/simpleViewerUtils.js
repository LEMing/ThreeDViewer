...
export const initializeCamera = (aspectRatio) => {
...
export const updateSize = (renderer, camera, mountRef) => {
    if (mountRef.current) {
...
export const fitCameraToObject = (scene, camera) => {
...
export const setupScene = (threeBase, object) => {
    const { mountRef, rendererRef, cameraRef } = threeBase;
    if (!mountRef.current)
        throw new Error('Mount div is not ready');
...
export const cleanupScene = (mountRef, renderer, resizeHandler) => {
